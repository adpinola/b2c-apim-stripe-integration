...
import {
  ApimProductIdKey,
  ApimSubscriptionIdKey,
  ApimSubscriptionNameKey,
} from "../constants";
...
export const register = (app: express.Application) => {
  app.post(`/api/v2/checkout/session`, async (req: any, res) => {
    const userEmail = req.body.userEmail;
    const apimSubscriptionName = req.body.apimUserId;
    ...
    const monetizationModel =
      await MonetizationService.getMonetizationModelFromProduct(apimProduct);
    ...
    const prices = (
      await stripe.prices.list({ product: apimProduct.name, active: true })
    ).data;
    ...
    async (req: express.Request, res: express.Response): Promise<void> => {
      ...
      switch (event.type) {
        case "customer.subscription.created":
          ...
          const apimSubscriptionName =
            stripeSubscription.metadata[ApimSubscriptionNameKey];

          const createSubscriptionResponse = await createApimSubscription(
            apimProductId,
            apimSubscriptionName
          );
          ...
          stripe.subscriptions.update(stripeSubscription.id, {
            metadata: {
              [ApimSubscriptionIdKey]: createSubscriptionResponse.name,
            },
          });
        ...
        case "customer.subscription.deleted":
          ...
          if (
            ...
            const apimSubscriptionId =
              stripeSubscription.metadata[ApimSubscriptionIdKey];
...
async function createApimSubscription(
  ...
  const apimService = new ApimService();
  return await apimService.createSubscription(
    apimProductId,
    apimSubscriptionName
  );
}
